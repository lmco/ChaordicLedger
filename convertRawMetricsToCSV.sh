...
do
    ...
    jq -r '.items[] | (.metadata.creationTimestamp + "," + .metadata.labels.app + "," + .containers[].usage.cpu + "," + .containers[].usage.memory)' ${absolutePath} | sed "s|n,|,|g" | sed "s|Ki||g" >> ${datafile}
done
...
sed -i 's|,\([0-9]*\)Mi$|,\1024|g' $datafile
